...
function isUserLoggedIn(moodCookValue, callerIP, callback) {
    if (moodCookValue !== undefined && callback !== undefined && typeof callback === 'function') {
        connection.query(`SELECT * FROM mdl_sessions WHERE sid LIKE '${moodCookValue}' AND timemodified > (UNIX_TIMESTAMP()-15*60*60) AND (firstip LIKE '%${callerIP}' OR lastip LIKE '%${callerIP}')`, (error, results, fields) => {
